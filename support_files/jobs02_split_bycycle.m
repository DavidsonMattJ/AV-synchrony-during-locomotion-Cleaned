% jobs_split_bycycle.
% Here we load the head-tracking data and apply the peak detection
% algorithm.
% calculate peaks/troughs are saved in the same participant triald data
% structure 'HeadPos'.
...
%% Set up
visualizeResults=0; % Show trial-by-trial peak detection
Fs=90; % sampling frequency of data.
...
%% Prepare to cycle through participant data:
cd(savedatadir);
% pfols
...
for ippant = 1:length(pfols)
    ...
    for  itrial=1:nTrials
        ...
        % extract trial data:
        useindx = find(framebyframe_table.trial == thistrial);

        trialtmp =  framebyframe_table.head_Y(useindx);        
        trialD_sm = smooth(trialtmp, 5); % small smoothing factor ~55 ms
        trialD = squeeze(trialtmp);
        Timevec = framebyframe_table.time(useindx);
        blockType= framebyframe_table.blockType(useindx(1));
        ...
        sumindx = find(summary_table.trial==thistrial);

        visOnsets= summary_table.vistargOnset(sumindx);
        audOnsets= summary_table.audCodedTime(sumindx);
        respOnsets=summary_table.targRT(sumindx);

        % store for later:
        HeadPos(itrial).Y = trialtmp;
        HeadPos(itrial).X = framebyframe_table.head_X(useindx);
        HeadPos(itrial).Z = framebyframe_table.head_Z(useindx);
        HeadPos(itrial).times = Timevec;
        HeadPos(itrial).walkSpeed = blockType;


        % Note that we can skip when ppant was stationary, during practice trials.
        if  ~isPrac && ~isStat
        ...
        % we may want to visualise the trial onsets and responses within each trial
        % as well:
        if visualizeResults
            ...
            plotD.figdir= figdir;
            plotD.subjID= subjID;
            plotD.walkSpeed= HeadPos(itrial).walkSpeed;
